...
Random Forest enables us to determine how important each variable is in the model, two criteria are used:\\•MeanDecreaseAccuracy - expresses the accuracy lost by leaving particular variable out of the training set. \\•MeanDecreaseGini - measures node impurity at each split, highest purity means that each node contains only elements of a single class.
```{r}
...
As we see, there are some variables we can try and exclude out of the model with expectations for better results. As for now, we check the performance of basic model. After that, we will take into consideration changes in the variables e.g. interactions or ommiting them.
...
pred1_RF <- prediction(as.numeric(predict(RanForest1, type = "response")), Airlines_binned_train$IsSatisfied)
...
predSc1_RF <- as.numeric(predict(RanForest1, type = "response"))
...
```
The model has an AUC of `r AUC1_RF` on training data and `r AUC_RanForest1_test` on testing data. As the difference between them is small we may presume that this model would be a good choice, plots also look satisfying. Just to make sure that we don't miss out on any better model, we decided to check some of the options for improvement. Excluding any variables resulted in making accuracy worse. Adding interactions didn't change accuracy or performance of the model at all. Our final choice is the basic model. We can now approach validation part.

### Validation
Similarly as in Logistic Regression we will validate the model using the cross validation method and opt for the Monte Carlo Cross Validation. As before, we will draw 100 times a training data-set containing 70% of observations and then study the AUC on the testing data-set.
```{r}
pctTrain <- 0.7
set.seed(1201984)
nRuns = 100
...
sd(AUCs_test_RF)
```
Observed values for the AUC of the test data have the median equal to `r median(AUCs_test_RF)`, the average equal to `r average(AUCs_test_RF)` and standard deviation equal to `r sd(AUCs_test_RF)`. Given the plots and parameters we are content with the results and find this model valid.
