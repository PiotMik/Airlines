...
```{r contdist, fig.pos="H", fig.height=4, fig.cap='Distributions of continuous variables.'}
hist
```
...
```{r Multicollinearity, echo=FALSE, fig.width=3.8, fig.height=3.8, fig.cap='\\label{fig:Multicollinearity} Correlation between continuous scale variables.'}
multicorr
```


```{r Delays_Collinearity, echo=FALSE, fig.width=2.6, fig.height=2.6, fig.cap='\\label{fig:Delays_Collinearity} Strong linear relationship between departure delay and arrival delay allows to drop one of them from the dataset.'}
...
```{r Loess, fig.width=5, fig.height=7.5, fig.cap = c('Loess estimator for IsSatisfied as function of continuous features from the dataset.\\label{fig:loess}')}
...
binnings$`WOE Table for eBookingNote` %>% 
  select(all_of(display_cols)) %>% knitr::kable(caption = "WOE table for eBookingNote.")
```
...
```{r CategoricalSpearman,  echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:spearman_cor} Spearman correlation shows no significant collinear relationships in ordinal variables", fig.pos="H"}
spearman_cor <-
  Airlines_binned %>%
...
RanForest1 <- if(file.exists(rf_model_path)){
                  ...
                  set.seed(12428)
                  randomForest(frm,Airlines_binned_train,
                  importance=TRUE) %T>%
...
```{r impvar, fig.height=4.5, fig.cap='Importance of variables measured by the mean decrease of accuracy and Gini score.'}
...
```{r KS_rf, fig.height=4, fig.pos="H", fig.cap='The KS Plot for the random forest.'}
ks_plot(actuals = Airlines_binned_train$IsSatisfied, predictedScores = predSc1_RF)
cat('\n\n')
```


```{r ROC_rf, fig.height=4, fig.pos="H", fig.cap='The ROC for the random forest.'}
...
```{r val_rf, fig.pos='H',fig.height=4, fig.cap='The results of the cross validation for the random forest.\\label{fig:MCxvalrf1}'}
...
```{r KS_logreg1, fig.pos="H", fig.height=4, fig.cap='The KS plot for the first logistic regression model.'}
ks_plot(actuals = Airlines_binned_train$IsSatisfied, predictedScores = predScores1)
```


```{r ROC_logreg1, fig.pos="H", fig.height=4, fig.cap='The ROC for the first logistic regression model.'}
...
```{r KS_logreg2, fig.pos="H", fig.height=4, fig.cap='The KS plot for the second logistic regression model.'}
ks_plot(actuals = Airlines_binned_train$IsSatisfied, predictedScores = predScores2)
```


```{r ROC_logreg2, fig.pos="H", fig.height=4, fig.cap='The ROC for the second logistic regression model.'}
...
```{r val_logreg2, fig.height=4,fig.cap='The histogram for the AUC of the randomised data-sets with the Monte Carlo cross validation for the second logistic regression model.\\label{fig:MCxval1}'}
...
```{r, include = TRUE,fig.height=4,fig.pos="H"}
...
```{r, include = TRUE,fig.height=4, fig.pos="H", fig.cap='The entire accuracy of the model was achieved in the first epoch of neural network training.'}
...
```{r KS_nn,fig.pos="H",fig.height=4,fig.cap='The KS for the neural network.'}
# Visualize the ROC curve:
...
```{r ROC_nn,fig.pos="H",fig.height=4,fig.cap='The ROC for the neural network.'}
...
```{r val_nn, fig.pos="H", fig.cap='The results of cross validation for the neural network.'}
...
As we can see, all challengers models performed well.
