...
```
```{r}
colnames(Airlines_binned)
```
```{r}
all_of(c(vars_to_drop))
```
```{r}
class(vars_to_drop)
```
...
test_acc <- filter(metrics_test, Metrics=="Accuracy")['Value'] %>% as.numeric()
...
Since Adaline's accuracy on the test set is `r round(test_acc, 3)`, we can infer that our data is not hard to separate, or at least it is relatively easy to reach a reasonable performance. For all challenger models we will be aiming to achieve better metrics, which must compensate for additional model complexity.
...
p1 <- ggplot(allAUCs, aes(AUC, fill = model, colour = model)) + geom_density(alpha=0.5) + xlim(0.9520, 0.9620)
...
In each layer we have used dropout = 0.3
...
fit_nn_model <- function(model, data_x, data_y) {
  history <- model %>%
      fit(data_x, data_y, epochs = 50, batch_size = 64,
...
Surprisingly, the neural network has achieved most of its performance after only one epoch.
This can be seen in the charts above.
...
### Validation
To keep the convention we have also used Monte Carlo Cross Validation
...
The AUC for the neural network model on training data have:
* the median equal to `r median(AUCs_test_RF)`,
* the average equal to `r mean(AUCs_test_RF)`,
* the standard deviation equal to `r sd(AUCs_test_RF)`.
