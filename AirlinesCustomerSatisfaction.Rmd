...
summary(AirlinesRaw)
```
In this project we will predict the satisfaction level of a customer in function
of the other variables based on past data. All results seems to be reasonable, but we notice that there are missing values in some columns. However, we will take care about it later.\\
...
```{r}
h1<-histogram(AirlinesRaw$satisfaction,AirlinesRaw,type='count',col='blue')
h2<-histogram(AirlinesRaw$Gender,AirlinesRaw,col='blue',type='count')
h3<-histogram(AirlinesRaw$`Customer Type`,AirlinesRaw,col='blue',type='count')
h4<-histogram(AirlinesRaw$Age,AirlinesRaw,col='blue',type='count')
h5<-histogram(AirlinesRaw$`Type of Travel`,AirlinesRaw,col='blue',type='count')
h6<-histogram(AirlinesRaw$Class,AirlinesRaw,col='blue',type='count')
h7<-histogram(AirlinesRaw$`Flight Distance`,AirlinesRaw,col='blue',type='count')
h8<-histogram(AirlinesRaw$`Seat comfort`,AirlinesRaw,col='blue',type='count')
h9<-histogram(AirlinesRaw$`Departure/Arrival time convenient`,AirlinesRaw,col='blue',type='count')
h10<-histogram(AirlinesRaw$`Food and drink`,AirlinesRaw,col='blue',type='count')
h11<-histogram(AirlinesRaw$`Gate location`,AirlinesRaw,col='blue',type='count')
h12<-histogram(AirlinesRaw$`Inflight wifi service`,AirlinesRaw,col='blue',type='count')
h13<-histogram(AirlinesRaw$`Inflight entertainment`,AirlinesRaw,col='blue',type='count')
h14<-histogram(AirlinesRaw$`Online support`,AirlinesRaw,col='blue',type='count')
h15<-histogram(AirlinesRaw$`Ease of Online booking`,AirlinesRaw,col='blue',type='count')
h16<-histogram(AirlinesRaw$`On-board service`,AirlinesRaw,col='blue',type='count')
h17<-histogram(AirlinesRaw$`Leg room service`,AirlinesRaw,col='blue',type='count')
h18<-histogram(AirlinesRaw$`Baggage handling`,AirlinesRaw,col='blue',type='count')
h19<-histogram(AirlinesRaw$`Checkin service`,AirlinesRaw,col='blue',type='count')
h20<-histogram(AirlinesRaw$Cleanliness,AirlinesRaw,col='blue',type='count')
h21<-histogram(AirlinesRaw$`Online boarding`,AirlinesRaw,col='blue',type='count')
...
grid.arrange(h13, h14, h15, h16,ncol=2)
grid.arrange(h17, h18, h19, h20,ncol=2)
grid.arrange(h21, h22, h23,ncol=2)
library("vioplot")
vioplot(AirlinesRaw)
plot(sample_n(AirlinesRaw, 500))
```
...
### Fitting and performance
The next model that we will study is a logistic regression. We name it for future reference \textit{logistic1}.
```{r}
# Define the formula:
frm <- IsSatisfied ~.
# Fit the model:
...
• the higher entertainment notes, seat notes, eBooking notes, eSupport notes, eBoarding, leg room notes, clean notes and baggage notes, the higher the satisfaction level
• satisfaction level is lower for low and medium service notes that for the high ones
• satisfaction level is higher for high check-in notes and lower for the low ones
• satisfaction level is higher for medium wifi notes than for the high ones
• satisfaction level is lower for high food notes than for the medium ones
```{r}
library(ROCR)
# function to get AUC
...
```{r}
frm2 <- IsSatisfied ~.+IsFemale*IsLoyal+IsFemale*Class+Class*IsLoyal+IsFemale*Age
# Fit the model:
...
```{r}
frm3 <- IsSatisfied ~.+IsFemale*IsLoyal+IsFemale*Class+Class*IsLoyal+IsFemale*Age-FlightDistance
# Fit the model:
logreg3 <- glm(formula = frm3, data = Airlines_binned_train, family = "binomial")
# Investigate the model:
summary(logreg3)
```
We notice that according to this model
...
• passengers at the age of 40s and 50s have a higher satisfaction level than the others
• satisfaction level is lower for long and medium flight distances than for the short ones
• the higher entertainment notes, seat notes, eBooking notes, eSupport notes, leg room notes, clean notes and baggage notes, the higher the satisfaction level
• satisfaction level is lower for low and medium service notes that for the high ones
• satisfaction level is higher for high check-in notes and lower for the low ones
• satisfaction level is higher for medium wifi notes than for the high ones
• satisfaction level is lower for high food notes than for the medium ones
• females travelling business class are less satisfied than the ones from eco plus class. Analogically, for loyal customers. 
...
AUC_logreg3_train <- fAUC(logreg3, data = Airlines_binned_train, type = "response")
AUC_logreg3_test <- fAUC(logreg3, data = Airlines_binned_test, type = "response")
pred3 <- prediction(predict(logreg3, type = "response"), Airlines_binned_train$IsSatisfied)

# Visualize the ROC curve:
plot(performance(pred3, "tpr", "fpr"), col="blue", lwd = 3)
AUC3 <- attr(performance(pred3, "auc"), "y.values")[[1]]
perf3 <- performance(pred3, "tpr", "fpr")
ks3 <- max(attr(perf3,'y.values')[[1]] - attr(perf3,'x.values')[[1]])
predScores3 <- predict(logreg3, type = "response")
ks_plot(actuals = Airlines_binned_train$IsSatisfied, predictedScores = predScores3)
cat('\n\n')
plotROC(actuals = Airlines_binned_train$IsSatisfied, predictedScores = predScores3)
```
...
```{r}
#The optimal cutoff:
optimalCutoff(actuals = Airlines_binned_train$IsSatisfied, predictedScores = predScores3, optimiseFor = "misclasserror")
```

The optimal cut-off (that gives the minimum mis-clasification error) is 0.48998. Above this cufoff level we assume satisfaction. 
...
mods <- map(cv_mc$train, ~ glm(frm3, data = ., family = "binomial"))
...
The median of the observed values for the AUC of the test data is 0.9571835, the average is 0.9571316 with a standard deviation of 0.0008522686.
