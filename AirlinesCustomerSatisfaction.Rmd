...
sShorterColnames <- c('IsSatisfied', 'IsFemale', 'IsLoyal', 'Age', 'IsBusinessTravel', 'Class',
                        'Distance', 'Seat', 'TimeConvenience', 'Victuals', 'Gate', 'WiFi',
                        'Entertainment', 'eSupport', 'eBooking', 'Onboarding', 'LegRoom',
                        'Baggage', 'CheckIn', 'Cleanliness', 'eBoarding', 'DepartureDelay', 'ArrivalDelay')

Airlines <- read_csv(sFileName,
                     col_types = "fffiffiffffffffffffffii")
colnames(Airlines) <- sShorterColnames
...
For that reason it is of utmost importance to pay extra care and attention to the data which is fed to the decision making models.
As the first step in our modeling pipeline we are going to look at the dataset to gain additional insight about it's statistics and information it conveys. We are going to perform quality checks on the data, such as outlier detection and treatment of not-available values (*NA*s). Lastly in this pre-modeling step we will look at the actual _information_ carried by particular features to remove variables of low added value.
