...
AirlinesEncoded %>% glimpse(width = 61)
```  
...
AirlinesEncoded$ArrivalDelay <- NULL
Airlines$ArrivalDelay <- NULL
```

Next, we're going to (very heuristically) examine the loess estimator of satisfaction as function of the remaining continuous variables to see if any of them looks flat enough to raise suspicion. Flatness of loess implies that average satisfaction does not change with the explanatory variable, hence the explanatory variable doesn't convey much information. 
In our case, this is not visible on the figure \ref{fig:loess}, so we cannot discard any feature based on that. 

```{r, eval=bRunComputationallyHeavyStuff, fig.width=5, fig.height=8, fig.caption = "Loess estimator for IsSatisfied as function of continuous features from the dataset.\\label{fig:loess}"}
...
For categorical features we will use the chi-square test and information value to
...
The Chi Square test rejected independency hypothesis of all categorical variables and the target variable. However, the table \ref{tab:CategoricalFeatureSelection} presents that there are features with IV smaller than $0.1$. That means even though they are co-dependent with satisfaction, their predictive power is not big. We are going to drop those from the dataset. 
...
Airlines$IsDelayed <- as_factor(ifelse(Airlines$DepartureDelay > 0, 1 , 0))
AirlinesEncoded$IsDelayed <- as_factor(ifelse(AirlinesEncoded$DepartureDelay > 0, 1 , 0))


sCategoricalFeatures <- 
  Airlines %>% 
  mutate(IsDelayed = as_factor(DepartureDelay>0)) %>%
  select(c(ends_with("Note"), starts_with("Is"), starts_with("Class"))) %>%
  colnames() %>% 
  setdiff(c("IsSatisfied"))

output <- data.frame(varName = character(0),
                     IV = numeric(0),
                     pvalChSq = numeric(0))  # create empty data frame

Airlines2 <- Airlines
Airlines2$IsDelayed <- as_factor(ifelse(Airlines$DepartureDelay > 0 , 1, 0)) 
for (colname in sCategoricalFeatures) {
   wt <- WOETable(Airlines2[[colname]], 
                  Airlines2$IsSatisfied, 
                  valueOfGood = 1)
   
   chisq_res <- Airlines2 %>% 
     select(all_of(c('IsSatisfied', colname))) %>% 
     table() %>% t() %>%
     chisq.test()
   
   tmp  <- data.frame(varName = colname, 
                      IV = sum(wt$IV),
                      pvalChSq = chisq_res$p.value)
...
knitr::kable(table_, 
             caption = "Categorical features of low informational value\\label{tab:CategoricalFeatureSelection}")
```

```{r}
AirlinesEncoded <- 
  AirlinesEncoded %>% select(-all_of(c(vars_to_drop)))

Airlines <- Airlines %>% select(-all_of(c(vars_to_drop)))
```
...
spearman_cor <-
  Airlines %>%
  select(all_of(sOrdinalFeatures)) %>%
  drop_na()%>%
  mutate_all(as.numeric) %>%
  cor(method='spearman')
...
Generally speaking we don't have any critical issue related to missing values in our data. Yes, there are *NAs* present in $12$ variables, but they constitute a minuscule portion of a very large dataset (see fig. \ref{tab:NA_structure}). We considered employing an imputation strategy based on median, but given that NAs constitute roughly `r round(mean(AirlinesEncoded %>% is.na() %>% rowSums() > 0), 2)` of all observations, even if we drop them we would still have `r (AirlinesEncoded %>% drop_na() %>% dim())[1]` observations left to work with. Based on that we decided not to introduce imputed values to the dataset, but rather work with pure data. 

```{r}
AirlinesEncoded <- drop_na(AirlinesEncoded)
...
Airlines_train <- Airlines[train_index, ]
Airlines_test  <- Airlines[test_index, ]

N2 <- nrow(AirlinesEncoded)
train_index2 <- sample(1:N2, 0.8 * N2)
test_index2  <- setdiff(1:N2, train_index)
AirlinesEncoded_train <- AirlinesEncoded[train_index2, ]
AirlinesEncoded_test  <- AirlinesEncoded[test_index2, ]
```
