...
First we will use the chi-square test and information value to detect dependency between the response variable and passenger note variables and quantify the predictive power of the categorical variables. It is customary to take information value smaller $0.1$ to mean "weak predictive power", so we will take that as our cut-off. 
...
for (colname in sCategoricalFeatures) {
   wt <- WOETable(Airlines[[colname]], 
                  Airlines$IsSatisfied, 
                  valueOfGood = 1)
...
```{r CategoricalSpearman,  echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:Spearman correlation for ordinal variables} Spearman correlation shows no significant colinear relationships in ordinal variables"}
sOrdinalFeatures <-
  Airlines %>% 
...
We have four continuous variables in our dataset: `Age`, `DepartureDelay`, `ArrivalDelay` and `Distance`. We start the analysis by noting high linear relationship between `ArrivalDelay` and `DepartureDelay`, visible on figure \ref{fig:Delays_Collinearity}. We can safely drop `ArrivalDelay`, since it doesn't introduce new information and also conaminates dataset with NAs.

```{r Delays_Collinearity, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:Delays_Collinearity} Strong linear relationship between departure delay and arrival delay allows to drop one of them from the dataset."}
