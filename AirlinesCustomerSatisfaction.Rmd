...
Therefore we will fit two simple models: a perceptron and a decision tree to the training set, and evaluate their performance using a reserved test set.
...
library(rpart)
library(rattle)
modelDecTree <- rpart(IsSatisfied ~ ., data = AirlinesEncoded_binned_train)
fancyRpartPlot(modelDecTree)


preds <- modelDecTree %>% predict(AirlinesEncoded_binned_test, type = "class")
preds <- preds %>% as.character() %>% as.numeric()


confusionMatrix(table(preds, unlist(AirlinesEncoded_binned_test$IsSatisfied,
                                    use.names = F)))
